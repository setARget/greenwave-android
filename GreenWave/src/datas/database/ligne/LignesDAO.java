...
public class LignesDAO extends DAOBase{
	...
	public long add(Ligne ligne){
		if(ligne !=  null){
			...
			values.put(LIGNES_ID, ligne.getIdBdd());
	...
	public void synchronisation(TreeMap<String, Ligne> tree){
		for (Map.Entry<String, Ligne> entry : tree.entrySet()) {
			if(entry.getValue() !=  null){
				Cursor c = db.query(LIGNES_TABLE_NAME, new String[] {LIGNES_ID, LIGNES_NBVUES, LIGNES_PREFRANG, LIGNES_FAVORIS}, LIGNES_ID + " LIKE \"" + entry.getValue().getIdBdd() +"\"", null, null, null, null);
				c.moveToFirst();
	...
	public void save(TreeMap<String, Ligne> tree){
		...
		for (Map.Entry<String, Ligne> entry : tree.entrySet()) {
			if(entry.getValue() !=  null){
				...
				db.update(LIGNES_TABLE_NAME, values, LIGNES_ID + " = " + entry.getValue().getIdBdd(), null);
			}
		}
	}
}
